...
class SendMessage():  # 定义发送消息的类
    def __init__(self):
        ...
        self.appID = os.environ["appID"]  # appid 注册时有
        self.appsecret = os.environ["appsecret"] # appsecret 同上
        self.template_id =  os.environ["template_id"]# 模板id
    ...
    def get_weather(self):
        ...
        感兴趣可以深入了解python爬虫
        """
        url = 'http://www.weather.com.cn/weather/101010100.shtml'  # 天气网站
    ...
    def get_herbirthday(self):
        ...
        herbirthDay = datetime.datetime(today.year, 1, 26)  # 将农历1.18号的时间转换为公历时间再转换为datetime类型的时间
        if herbirthDay > today:  # 如果ta的生日日期比今天靠后则直接计算这两天的序号之差
            difference = herbirthDay.toordinal() - today.toordinal()
            return ("\n距离糗糗生日,还有 %d 天。" % (difference))
    ...
    def get_mybirthday(self):
        ...
        data_str = today.strftime('%Y-%m-%d')
        mybirthDay = datetime.datetime(today.year, 1, 5)
        if mybirthDay > today:
            difference = mybirthDay.toordinal() - today.toordinal()
            return ("\n距离旺仔生日,还有 %d 天。" % (difference))
    ...
    def get_loveday(self):
        ...
        today = datetime.datetime.now()
        data_str = today.strftime('%Y-%m-%d')
        oneDay = datetime.date(2022, 1, 17)
